...
# -*- coding: utf-8 -*-
"""
OpenVINO GenAI Qwen3-30B-A3B 性能基准测试

测量 Qwen3-30B-A3B 模型的推理速度 (tokens/s)
对比 Ollama 的性能
"""
...
DEFAULT_MODEL_PATH = os.path.join(os.path.dirname(__file__), "Qwen3-30B-A3B-int4-ov")
...
def quick_test(model_path: str, device: str = "GPU", prompt: str = None):
    ...
    print("=" * 70)
    print("🚀 OpenVINO GenAI Qwen3-30B-A3B 性能测试")
    print("=" * 70)
    ...
    # 与 Ollama 对比
    ollama_speed = 11.19  # 之前测试的 Ollama qwen3:30b-a3b 速度
    print("\n📊 与 Ollama 对比:")
    print("-" * 70)
    ...
    if tokens_per_sec > ollama_speed:
        ...
        slowdown = ollama_speed / tokens_per_sec
        print(f"   ⚠️ OpenVINO 慢了约 {slowdown:.1f}x")
    else:
        print(f"   ⚖️ 速度相当")
    print("=" * 70)
...
def benchmark_inference(model_path: str, device: str = "GPU", max_new_tokens: int = 256, num_runs: int = 3):
    ...
    print("=" * 70)
    print("🚀 OpenVINO GenAI Qwen3-30B-A3B 性能基准测试")
    print("=" * 70)
    print(f"📦 OpenVINO GenAI 版本: {ov_genai.__version__}")
    print(f"📁 模型路径: {model_path}")
    print(f"💻 运行设备: {device}")
    ...
    print("-" * 70)
    print(f"   Ollama qwen3:30b-a3b (GPU):     {ollama_speed:.2f} tokens/s")
    print(f"   OpenVINO Qwen3-30B-A3B ({device}): {overall_avg_tps:.2f} tokens/s")
    if overall_avg_tps > ollama_speed:
        speedup = overall_avg_tps / ollama_speed
        print(f"   🚀 OpenVINO 快了约 {speedup:.1f}x !")
    print("=" * 70)
    
    return {
        "model": "Qwen3-30B-A3B-int4-ov",
        "device": device,
        "load_time": load_time,
...
def main():
    parser = argparse.ArgumentParser(description="OpenVINO GenAI Qwen3-30B-A3B 性能基准测试")
    parser.add_argument(
        "--model-path",
        type=str,
    ...
    if not os.path.exists(args.model_path):
        ...
        print("💡 请先运行 download_qwen3_30b_a3b.py 转换模型")
        sys.exit(1)
